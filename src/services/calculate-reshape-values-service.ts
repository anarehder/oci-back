export function estimateCPUreduce(maxCPU: number, meanCPU: number, OCPU: number) {
    let newOCPU = OCPU;
...
export function estimateCPUincrease(maxCPU: number, meanCPU: number, OCPU: number) {
    let newOCPU = OCPU;
...
export function estimateMEMreduce(maxMEM: number, MEM: number) {
    let estimatedMEM = ((Number(maxMEM) * 1.1) / 100) * Number(MEM);
    estimatedMEM = Math.ceil(estimatedMEM);
    return estimatedMEM;
}

export function estimateMEMincrease(meanMEM: number, MEM: number) {
