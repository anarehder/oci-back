...
import { get30MinAverageCPUByTenancyRepository, get30MinAverageCPURepository } from "../repositories";
...
export async function getLatestCpusService(userToken: string) {
    ...
    if (userDetails[0].isAdmin){
        const response = await get30MinAverageCPURepository();
        return response;
    } else{
        const tenancies = userDetails.map(user => user.tenancy);
        const response = await get30MinAverageCPUByTenancyRepository(tenancies);
        return response;
    }
